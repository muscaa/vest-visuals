...
export interface MediaVariantsConfig {
    table: MediaVariantsTable;
    bucket: string;
}

export function format(config: MediaVariantsConfig, props: types.SelectProps): types.MediaVariant {
    return {
        contentId: props.contentId,
        tag: props.tag,
        order: props.order,
        fileUrl: `${serverConfig.env.S3_URL}/${config.bucket}/${filePath(props.contentId, props.tag)}`,
        type: props.type,
...
export async function getAll(config: MediaVariantsConfig): Promise<types.MediaVariant[]> {
    const result = await db.select()
        .from(config.table)
        .orderBy(
            asc(config.table.contentId),
            asc(config.table.order),
        )
        .all();
    return result.map((value) => format(config, value));
}

export async function getArray(config: MediaVariantsConfig, contentId: string): Promise<types.MediaVariant[]> {
    const result = await db.select()
        .from(mediaVariants)
