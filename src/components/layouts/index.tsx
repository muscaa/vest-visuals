...
export function defaultMetadata(t: _Translator<Record<string, any>, never>, type: "Media" | "Software", name: string, url: Pathname): MetadataResult {
    return {
        title: t(`${type}.Metadata.${name}.title`),
        description: t(`${type}.Metadata.${name}.description`),
        base: type == "Media" ? sharedConfig.env.MEDIA_URL : type == "Software" ? sharedConfig.env.SOFTWARE_URL : "",
        url,
        image: og.splitImage({
            title: t(`${type}.Metadata.${name}.og-title`),
            subtitle: t(`${type}.Metadata.${name}.og-subtitle`),
            image: type == "Media" ? "/media.jpg" : type == "Software" ? "/software.jpg" : "/placeholder.jpg",
        }),
    };
}
